...
class MemberLogic extends Controller
{
	public static function select($post,$table)
	{
	    ...
	    $list=Db::name('members')->alias('m')->field(['m.id','m.username','m.sex','m.create_time','m.type','m.status','m.email','m.address','m.telephone'])->page($page,$limit)->where($where)->where($time)->select();
