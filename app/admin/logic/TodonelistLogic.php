...
class TodonelistLogic extends Controller
{
	public static function select($post,$table)
	{
	    ...
	    $count=Db::name($table)->where('sponsor|approve','=',Session::get('login.name'))->where($where)->where($time)->count();

	    $list=Db::name($table)->page($page,$limit)->where('sponsor|approve','=',Session::get('login.name'))->where($where)->where($time)->order('id desc')->select();
