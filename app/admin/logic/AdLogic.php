...
class AdLogic extends Controller
{
	public static function select($post,$table)
	{
	    ...
	    $list=Db::name('ad')->alias('a')->field('a.*,a.id aid,a.name aname,a.status astatus,p.*,p.id pid,p.name pname,p.status pstatus')->join('ad_position p','a.position_id=p.id','LEFT')->order('aid')->page($page,$limit)->where($where)->where($time)->select();

	    $count=Db::name('ad')->alias('a')->field('a.*,a.id aid,a.name aname,a.status astatus,p.*,p.id pid,p.name pname,p.status pstatus')->join('ad_position p','a.position_id=p.id','LEFT')->where($where)->where($time)->count();
