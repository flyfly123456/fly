<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:49:"../app/admin/view/attendance\attendance_list.html";i:1548850896;}*/ ?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
...
<body class="layui-anim layui-anim-up">
    ...
    <div class="x-body">
        ...
        <table class="layui-table" lay-data="{height:'full-100', cellMinWidth: 80, url:'attendance_list_info', toolbar:'#demoTable', page:true, id:'idTest',initSort: {field:'id', type:'desc'}}" lay-filter="archives">
            <thead>
                <tr>
                    <th lay-data="{type:'checkbox', fixed: 'left'}"></th>
                    <th lay-data="{field:'id',sort: true, fixed: true}">ID</th>
                    <th lay-data="{field:'dates', width:140}">日期</th>
                    <th lay-data="{field:'weeks', width:140}">星期</th>
                    <th lay-data="{field:'admin', width:140}">名称</th>
                    <th lay-data="{field:'start', width:160}">上班时间</th>
                    <th lay-data="{field:'end', width:160}">下班时间</th>
                    <th lay-data="{field:'department', width:160}">部门</th>
                    <th lay-data="{field:'desc', width:300}">描述</th>
                    <th lay-data="{fixed: 'right',field:'status', width:110, templet: '#statusTpl'}">状态</th>
    ...
    <script type="text/html" id="demoTable">
        ...
        <button class="layui-btn layui-btn-sm layui-btn-warm" onclick="x_admin_show('申请补卡','apply_clock')"><i class="layui-icon layui-icon-praise"></i>申请补卡</button>
    </script>

    <script type="text/html" id="barDemo">
        <a class="layui-btn layui-btn-xs" lay-event="detail" onclick="x_admin_show('清除考勤异常','clear_exception?dates={{d.dates}}')">清除异常</a>
    </script>
    ...
    layui.use(['form','layer','laydate','table'], function(){
        ...
        var $ = layui.$, active = {
            ...
            ,reload: function(){
                ...
                console.log(post)
                $.ajax({
                    url: "<?php echo url('admin/Attendance/attendance_list_info'); ?>",
                    type: 'POST',
                    dataType: 'json',
                    data: post,
                    success: function(res) {
            ...
            ,word_export:function(){
                ...
                if(data.length==0){
                    ...
                    layer.confirm('确认要删除吗？',{icon: 3, title:'提示'},function(index){
                        ...
                        // console.log(data)
                        $.ajax({
                            url: "<?php echo url('admin/Staff/word_export'); ?>",
                            type: 'POST',
                            dataType: 'json',
                            data: jsonObj,
                            success: function(res) {
                                ...
                                if(res.status==1){
                                    layer.closeAll();
                                    window.location.href="<?php echo url('admin/Staff/download_word'); ?>?filenamedoc="+res.msg;
                                }else{
        ...
        form.on('switch(editSex)', function(obj){
            ...
            $.ajax({
                url: "<?php echo url('admin/Staff/archives_edit'); ?>",
                type: 'POST',
                dataType: 'json',
                data: post,
                success: function(res) {
    ...
    function f_archives_export(){
      layer.confirm('确认要导出吗？',function(){
          window.location="<?php echo url('admin/Staff/export'); ?>";
