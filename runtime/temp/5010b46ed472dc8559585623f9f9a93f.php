<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:38:"../app/admin/view/shop\model_list.html";i:1551188135;}*/ ?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
...
<body class="layui-anim layui-anim-up">
    ...
    <div class="x-body">
        ...
        <table class="layui-table" lay-data="{height:'full-100', cellMinWidth: 80, url:'model_list_info', toolbar:'#demoTable', page:true, id:'idTest',initSort: {field:'id', type:'desc'}}" lay-filter="admin">
            <thead>
                <tr>
                    ...
                    <th lay-data="{field:'param', width:300}">参数</th>
                    <th lay-data="{fixed: 'right', field:'is_available', width:110, templet: '#is_available'}">状态</th>
    ...
    <script type="text/html" id="demoTable">
        ...
        <button class="layui-btn layui-btn-sm" onclick="x_admin_show('添加模型','model_add')"><i class="layui-icon">&#xe608;</i>添加</button>
    ...
    <script type="text/html" id="barDemo">
        <a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail" onclick="x_admin_show('查看','model_view?id={{d.id}}')">查看</a>
        <a class="layui-btn layui-btn-xs" lay-event="edit" onclick="x_admin_show('模型编辑','model_edit?id={{d.id}}')">编辑</a>
        <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">删除</a>
    </script>

    <script type="text/html" id="is_available">
        <input type="checkbox" name="is_available" value="{{d.id}}" title="正常" lay-filter="deitStatus" {{ '正常' == d.is_available ? 'checked' : '' }}>
    </script>
    ...
    layui.use(['form','layer','laydate','table'], function(){
        ...
        var $ = layui.$, active = {
            ...
            ,reload: function(){
                ...
                console.log(post)
                $.ajax({
                    url: "<?php echo url('admin/Shop/model_list_info'); ?>",
                    type: 'POST',
                    dataType: 'json',
                    data: post,
                    success: function(res) {
        ...
        form.on('checkbox(deitStatus)', function(obj){
            ...
            $.ajax({
                url: "<?php echo url('admin/Shop/shop_edit'); ?>",
                type: 'POST',
                dataType: 'json',
                data: post,
                success: function(res) {
