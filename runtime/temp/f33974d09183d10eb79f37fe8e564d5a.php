<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:39:"../app/admin/view/wechat\fans_list.html";i:1547725604;}*/ ?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
...
<body class="layui-anim layui-anim-up">
    ...
    <div class="x-body">
        ...
        <table class="layui-table" lay-data="{height:'full-100', cellMinWidth: 80, url:'fans_list_info', toolbar:'#demoTable', page:true, id:'idTest',initSort: {field:'id', type:'desc'}}" lay-filter="fans">
            <thead>
                <tr>
                    <th lay-data="{type:'checkbox', fixed: 'left'}"></th>
                    <th lay-data="{field:'id',sort: true, fixed: true}">ID</th>
                    <th lay-data="{field:'subscribe', width:140}">关注</th>
                    <th lay-data="{field:'openid',width:180}">openid</th>
                    <th lay-data="{field:'nickname', width:140}">昵称</th>
                    <th lay-data="{field:'sex', width:140}">性别</th>
                    <th lay-data="{field:'country', width:140}">国家</th>
                    <th lay-data="{field:'city', width:140}">城市</th>
                    <th lay-data="{field:'province', width:140}">省份</th>
                    <th lay-data="{field:'headimgurl',templet:'#headimg'}">头像</th>
                    <th lay-data="{field:'subscribe_time', width:180, sort: true}">关注时间</th>
                    <th lay-data="{field:'subscribe_scene', width:180}">关注方式</th>
    ...
    <script type="text/html" id="demoTable">
        ...
        <button class="layui-btn layui-btn-sm layui-btn-warm" data-type="fans_tag_list" onclick="x_admin_show('粉丝标签','fans_tag_list')"><i class="layui-icon layui-icon-add-circle-fine"></i>粉丝标签</button>
        ...
        <button class="layui-btn layui-btn-sm layui-btn-normal push_message_all" data-type="push_message_all" onclick="x_admin_show('批量推送','push_message_all')"><i class="layui-icon layui-icon-speaker"></i>批量推送</button>
    </script>

    <script type="text/html" id="barDemo">
        <a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail" onclick="x_admin_show('编辑粉丝','fans_edit?id={{d.id}}&openid={{d.openid}}')">编辑</a>
    ...
    <script type="text/html" id="headimg">
        <div style="width:100%;height:100%;text-align:center;"><img onclick="view_headimg('{{d.headimgurl}}')" style="width:30px;height:30px;" title="点击查看大图" src="{{d.headimgurl}}" /></div>
    </script>
    ...
    layui.use(['form','layer','laydate','table'], function(){
        ...
        //监听单元格编辑
        table.on('edit(fans)', function(obj){
        ...
        //监听工具条
        table.on('tool(fans)', function(obj){
            var data = obj.data;
            if(obj.event === 'detail'){
        ...
        table.on('row(fans)', function(obj){
        ...
        var $ = layui.$, active = {
            ...
            ,full_fans_list:function(){
                layer.confirm('确认要拉取所有微信粉丝？',{icon: 3, title:'提示'},function(index){
                    ...
                    $.ajax({
                        url: "<?php echo url('admin/Wechat/fans_list'); ?>",
                        type: 'POST',
                        dataType: 'json',
                        data: {pull:1},
                        success: function(res) {
