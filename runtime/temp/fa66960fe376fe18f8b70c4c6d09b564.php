<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:37:"../app/admin/view/shop\shop_list.html";i:1552485483;}*/ ?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
...
<body class="layui-anim layui-anim-up">
    ...
    <div class="x-body">
        ...
        <table class="layui-table" lay-data="{height:'full-100', cellMinWidth: 80, url:'shop_list_info', toolbar:'#demoTable', page:true, id:'idTest',initSort: {field:'id', type:'desc'}}" lay-filter="admin">
            <thead>
                <tr>
                    ...
                    <th lay-data="{field:'brand',width:180}">品牌</th>
                    <th lay-data="{field:'desc', width:300}">描述</th>
                    <th lay-data="{field:'parameter', width:300}">参数</th>
                    <th lay-data="{field:'price', width:120}">价格</th>
                    <th lay-data="{field:'number', width:180}">库存</th>
                    <th lay-data="{field:'create_time', width:180}">添加时间</th>
                    <th lay-data="{fixed: 'right', field:'shelf', width:110, templet: '#shelf'}">状态</th>
    ...
    <script type="text/html" id="demoTable">
        ...
        <button class="layui-btn layui-btn-sm" onclick="x_admin_show('添加用户','shop_add')"><i class="layui-icon">&#xe608;</i>添加</button>
    ...
    <script type="text/html" id="barDemo">
        <a class="layui-btn layui-btn-xs" lay-event="edit" onclick="x_admin_show('商品编辑','shop_edit?id={{d.id}}')">编辑</a>
        <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">删除</a>
    </script>

    <script type="text/html" id="shelf">
        <input type="checkbox" name="shelf" value="{{d.id}}" title="上架" lay-filter="deitStatus" {{ '1' == d.shelf ? 'checked' : '' }}>
    </script>
    ...
    layui.use(['form','layer','laydate','table'], function(){
        ...
        var $ = layui.$, active = {
            batchdelete: function(){ //批量删除
                ...
                if(data.length==0){
                    ...
                    layer.confirm('确认要删除吗？',{icon: 3, title:'提示'},function(index){
                        // console.log(data)
                        $.ajax({
                            url: "<?php echo url('admin/shop/delete_all'); ?>",
                            type: 'POST',
                            dataType: 'json',
                            data: {id:data},
                            success: function(res) {
            ...
            ,reload: function(){
                ...
                console.log(post)
                $.ajax({
                    url: "<?php echo url('admin/Shop/shop_list_info'); ?>",
                    type: 'POST',
                    dataType: 'json',
                    data: post,
                    success: function(res) {
        ...
        form.on('checkbox(deitStatus)', function(obj){
            ...
            console.log(post)
            $.ajax({
                url: "<?php echo url('admin/shop/change_status'); ?>",
                type: 'POST',
                dataType: 'json',
                data: post,
                success: function(res) {
